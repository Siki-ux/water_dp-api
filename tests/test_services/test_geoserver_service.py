...
class TestGeoServerService:
    ...
    def test_create_workspace_new(self, mock_request, service):
        ...
        mock_response_check.raise_for_status.side_effect = requests.exceptions.HTTPError(response=mock_response_check)
    ...
    def test_publish_layer(self, mock_request, service):
        ...
        with patch.object(service, 'set_layer_style') as mock_set_style:
    ...
    def test_publish_sql_view(self, mock_request, service):
        ...
        mock_check.raise_for_status.side_effect = requests.exceptions.HTTPError(response=mock_check)
    ...
    def test_create_datastore(self, mock_request, service):
        ...
        mock_check = MagicMock()
        mock_check.status_code = 404
        mock_check.raise_for_status.side_effect = Exception("Not Found") # Requests usually raises HTTPError but logic handles general check
        ...
        result = service.create_datastore("new_store", connection_params={"host": "localhost"})
        assert result is True
    ...
    def test_create_style(self, mock_request, service):
        ...
        assert result is True
        assert mock_request.call_args[1]['headers']['Content-Type'] == 'application/vnd.ogc.sld+xml'
    ...
    def test_sync_layer_with_database(self, mock_request, service):
        ...
        with patch.object(service, 'create_datastore', return_value=True), \
             patch.object(service, 'publish_layer', return_value=True):
