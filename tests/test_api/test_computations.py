...
def test_trigger_computation_success(
    mock_check_access, mock_exists, mock_delay, mock_db_session, mock_auth
):
    # Setup
    script_id = uuid4()
    project_id = uuid4()
    ...
    response = client.post(
        f"/api/v1/computations/run/{script_id}", json={"params": {"location_id": 1}}
    )
