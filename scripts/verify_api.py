...
def test_layers():
    ...
    try:
        ...
        found = any(l['layer_name'] == 'czech_regions' for l in data['layers'])
        if found:
...
def test_features():
    ...
    try:
        ...
        r = requests.get(f"{BASE_URL}/geospatial/features?layer_name=czech_regions&limit=5")
        assert r.status_code == 200, f"Status: {r.status_code}, Body: {r.text}"
        data = r.json()
        ...
        if len(features) > 0:
            ...
            if station_id:
                ...
                r_single = requests.get(f"{BASE_URL}/geospatial/features/{f['feature_id']}?layer_name=czech_regions")
                assert r_single.status_code == 200, f"Status: {r_single.status_code}, Body: {r_single.text}"
