...
def import_sensors(cursor, db_connection, project_id, user_id):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code != 200:
                logger.warning(f"Failed to fetch Things (Attempt {attempt+1}/{max_retries}): {resp.status_code}")
            else:
                ...
                if len(things) >= min_sensors_required:
                    ...
                    logger.info(f"Only {len(things)} Things found (Attempt {attempt+1}/{max_retries}). Waiting for more sensors...")
        except Exception as e:
            logger.warning(f"Connection error checking FROST (Attempt {attempt+1}/{max_retries}): {e}")
        time.sleep(retry_delay)
    
    if not things:
        logger.warning(f"No sensors found in FROST after {max_retries*retry_delay} seconds. Skipping import.")
        return
    ...
    for t in things:
            ...
            if not existing_id:
                # Try by Name
                cursor.execute('SELECT id, uuid FROM "thing" WHERE name = %s AND project_id = %s', (t_name, project_id))
                row = cursor.fetchone()
                if row:
                    existing_id = row['id']
            ...
            if not existing_id:
                ...
                try:
                    cursor.execute(
                        ...
                        """,
                        (t_name, t.get("description", "Imported Sensor"), new_uuid, project_id, 2, user_id, datetime.datetime.now(datetime.timezone.utc))
                    )
            ...
            if updates_needed:
                logger.info(f"Updating FROST properties for {t_name} (Enabling Simulation)...")
                try:
                    ...
                    if r_patch.status_code not in [200, 204]:
                        logger.error(f"Failed to update FROST thing {t_name}: {r_patch.text}")
                except Exception as e:
...
def run_seed():
    ...
    if user_row:
        ...
        try:
            cursor.execute(
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id
                """,
                ('admin-siki', 'dummy', 'admin@example.com', 'Admin', 'Siki', 'Admin Siki', 'admin-siki-uuid-1234')
            )
    ...
    row = cursor.fetchone()
    if row:
        logger.info(f"Project '{group_name}' already exists (ID: {row['id']}). Skipping creation.")
        project_id = row['id']
    else:
    ...
    cursor.execute("SELECT id FROM file_parser WHERE name = %s AND project_id = %s", (parser_name, project_id))
    parser_row = cursor.fetchone()
    
    if parser_row:
        logger.info(f"File Parser '{parser_name}' already exists (ID: {parser_row['id']}).")
    else:
        ...
        try:
            cursor.execute(
                ...
                """,
                (project_id, parser_name, settings, csv_parser_type_id, user_id, datetime.datetime.now(datetime.timezone.utc))
            )
