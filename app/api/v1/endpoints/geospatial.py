...
from app.schemas.geospatial import (
    GeoLayerCreate, GeoLayerResponse, GeoLayerUpdate,
    GeoFeatureCreate, GeoFeatureResponse, GeoFeatureUpdate,
    LayerQuery, FeatureQuery, SpatialQuery,
    LayerListResponse, FeatureListResponse, SpatialQueryResponse,
    LayerPublishRequest, LayerUnpublishRequest
)
...
async def create_geo_layer(
    layer: GeoLayerCreate,
...
async def get_geo_layers(
    ...
    try:
        ...
        if is_published is not None:
            layers = [l for l in layers if l.is_published == str(is_published).lower()]
...
async def get_geo_layer(
    ...
    try:
        ...
        if not layer:
            raise HTTPException(status_code=404, detail=f"Geo layer {layer_name} not found")
        return layer
...
async def get_geo_feature(
    ...
    try:
        ...
        if not feature:
            raise HTTPException(status_code=404, detail=f"Geo feature {feature_id} not found in layer {layer_name}")
        return feature
    except HTTPException:
        raise
    except Exception as e:
...
async def unpublish_layer_from_geoserver(
    request: LayerUnpublishRequest,
    db: Session = Depends(get_db)
):
    """Unpublish a layer from GeoServer."""
    try:
        geoserver_service = GeoServerService()
        success = geoserver_service.unpublish_layer(request.layer_name, request.workspace)
