...
from fastapi import APIRouter, Depends, HTTPException, Query, status, File, UploadFile, Path, Request
    ...
    for sid in sensor_ids:
        try:
            ...
            if latest_data:
                ...
                if datetime.now(last_activity.tzinfo) - last_activity < timedelta(hours=24):
                    is_active = True
...
def create_project_thing(
    ...
    # Link to Project
    try:
        ProjectService.add_sensor(db, project_id, created_station_dict["station_id"], current_user)
    except Exception as e:
        # Rollback: Delete station?
        # For now just fail.
        raise HTTPException(status_code=500, detail=f"Failed to link station to project: {e}")

    # Return response
    return StationResponse(
        **created_station_dict,
        is_active=False,
        last_activity=None
    )
...
def update_project_thing(
    ...
    updated = ts_service.update_station(station_id_str, thing_in.model_dump(exclude_unset=True))
    if not updated:
        raise HTTPException(status_code=404, detail="Station not found")
        
    return StationResponse(
        **updated,
        is_active=False, # Re-calc?
        last_activity=None
    )
...
def delete_project_thing(
    project_id: UUID,
    thing_id: str,
    delete_from_source: bool = Query(False, description="If true, deletes from FROST Server too."),
    db: Session = Depends(get_db),
    current_user: dict = Depends(deps.get_current_user),
    ts_service: TimeSeriesService = Depends(deps.get_time_series_service),
) -> Any:
        ...
        if station:
             ...
             if found_id in allowed_sensors:
                 ...
                 if found_sid and str(found_sid) in allowed_sensors:
                     ...
                     raise HTTPException(status_code=404, detail="Thing not found in project (Permission Denied)")
        else:
...
from app.services.time_series_service import ResourceNotFoundException
...
async def import_project_thing_file(
    ...
    thing_id: Annotated[str, Path(description="Thing ID (FROST @iot.id) or Station ID Property")],
    file: Annotated[UploadFile, File(description="Data file (CSV or JSON)")],
    parameter: Annotated[str, Query(description="The parameter (ObservedProperty) to import data for (e.g. Level, Flow, Temperature).")],
    ...
    """
    from app.services.time_series_service import TimeSeriesService, ResourceNotFoundException
    from app.schemas.time_series import TimeSeriesDataCreate
    ...
    try:
        ...
        imported_count = ts_service.add_bulk_data(series_id, data_points)
         ...
         raise HTTPException(status_code=404, detail=f"Datastream {series_id} could not be created or found.")
...
async def import_project_thing_json(
    ...
    parameter: Annotated[str, Query(description="The parameter (ObservedProperty) to import data for.")],
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[dict, Depends(deps.get_current_user)],
) -> Any:
    """
