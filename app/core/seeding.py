...
def generate_grid_polygons(bbox: Tuple[float, float, float, float], rows: int = 3, cols: int = 4) -> List[Polygon]:
    ...
    try:
        if not data_exists:
            ...
            if os.path.exists(geojson_path):
                ...
                try:
                    ...
                    for idx, feature_data in enumerate(features):
                        ...
                        region_name = props.get('name') or props.get('NAZ_CZNUTS3') or f"Region_{idx+1}"
                        feature_id_raw = props.get('id') or feature_data.get('id') or f"region_{idx+1}"
            ...
            if not region_features:
                logger.info("Generating synthetic grid regions (GeoJSON missing or failed).")
            ...
            if not db.query(GeoLayer).filter(GeoLayer.layer_name == "czech_republic").first():
                 db.add(cz_rep_layer)
                 db.flush()

                 cz_rep_geojson_path = os.path.join(data_dir, "czech_republic.json")
                 if not os.path.exists(cz_rep_geojson_path):
                     cz_rep_geojson_path = os.path.join(data_dir, "czech_republic.geojson")
                 ...
                 if os.path.exists(cz_rep_geojson_path):
                      ...
                      try:
                          ...
                          for idx, feature_data in enumerate(cz_features):
                              props = feature_data.get('properties', {})
                              feature_id_raw = props.get('id') or feature_data.get('id') or f"cz_rep_{idx}"
            ...
            for feature, poly in region_features:
                region_name = feature.properties.get('name', f"Region_{feature.feature_id}")
            ...
            for station in stations:
                ...
                while current_time <= now:
                    # Seconds from start
                    t = (current_time - start_time).total_seconds() / (24 * 3600) # days
                    
                    # Value calculation
                    val = base_level + amplitude * math.sin(2 * math.pi * frequency * t + phase)
                    # Add noise
        ...
        try:
            ...
            if gs_service.test_connection():
                ...
                gs_service.create_datastore(store_name, connection_params=connection_params)
                ...
                sql_cz = f"SELECT * FROM geo_features WHERE layer_id = '{layer_name_cz}'"
                ...
                logger.info(f"Successfully published layer {layer_name_cz} to GeoServer")
            else:
