...
class TimeSeriesMetadata(Base, BaseModel):
    ...
    parameter = Column(String(50), nullable=False, index=True)  # water_level, temperature, etc.
...
class TimeSeriesData(Base, BaseModel):
    ...
    # Data quality
    quality_flag = Column(String(20), default="good")  # good, questionable, bad, missing
...
class TimeSeriesAggregation(PydanticBase):
    ...
    start_time: datetime
    end_time: datetime
    aggregation_method: str = Field(..., pattern="^(mean|max|min|sum|count|std|median)$")
    aggregation_interval: str = Field(..., pattern="^(1min|5min|15min|1hour|1day|1week|1month)$")
    time_zone: str = "UTC"
