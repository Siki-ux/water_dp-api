...
from app.schemas.geospatial import (
    GeoLayerCreate, GeoFeatureCreate, LayerPublishRequest,
    GeoServerLayerInfo, GeoServerStyleInfo
)
...
class GeoServerService:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to GeoServer version: {version_info.get('version', 'unknown')}")
            return True
    ...
    def create_workspace(self, workspace_name: str = None) -> bool:
        ...
        try:
            ...
            response = self._make_request('GET', f'/workspaces/{workspace_name}.json')
    ...
    def create_datastore(self, store_name: str, store_type: str = "postgis", 
                        connection_params: Dict[str, Any] = None) -> bool:
        """Create data store."""
        try:
            # Check if store exists
            response = self._make_request('GET', f'/workspaces/{self.workspace}/datastores/{store_name}.json')
            ...
            if e.response.status_code == 404:
                ...
                response = self._make_request(
                    'POST', f'/workspaces/{self.workspace}/datastores.json',
                    json=store_data
                )
    ...
    def publish_layer(self, layer_request: LayerPublishRequest) -> bool:
        ...
        try:
            ...
            response = self._make_request(
                'POST', f'/workspaces/{layer_request.workspace}/datastores/{layer_request.store_name}/featuretypes.json',
                json=layer_config
            )
    ...
    def publish_sql_view(self, layer_name: str, store_name: str, sql: str, 
                        workspace: str = None, title: str = None) -> bool:
        ...
        try:
            ...
            try:
                self._make_request('GET', f'/workspaces/{workspace}/layers/{layer_name}.json')
    ...
    def set_layer_style(self, layer_name: str, style_name: str, 
        ...
        try:
            ...
            response = self._make_request(
                'PUT', f'/workspaces/{workspace}/layers/{layer_name}.json',
                json=style_config
            )
    ...
    def create_style(self, style_name: str, sld_content: str, 
                    workspace: str = None) -> bool:
        ...
        try:
            ...
            # Create style
            response = self._make_request(
                'POST', f'/workspaces/{workspace}/styles.json',
                json=style_config
            )
            
            # Upload SLD content
            response = self._make_request(
                'PUT', f'/workspaces/{workspace}/styles/{style_name}.sld',
                data=sld_content,
    ...
    def get_layer_info(self, layer_name: str, workspace: str = None) -> Optional[GeoServerLayerInfo]:
        """Get layer information from GeoServer."""
        workspace = workspace or self.workspace
        
        try:
            response = self._make_request('GET', f'/workspaces/{workspace}/layers/{layer_name}.json')
            ...
            return GeoServerLayerInfo(
                ...
                native_srs=layer_data['layer']['resource'].get('nativeSRS', 'EPSG:4326'),
    ...
    def get_layer_capabilities(self, layer_name: str, workspace: str = None) -> Dict[str, Any]:
    ...
    def generate_wms_url(self, layer_name: str, workspace: str = None, 
                         bbox: Tuple[float, float, float, float] = None,
                         width: int = 256, height: int = 256,
                         srs: str = "EPSG:4326", format: str = "image/png") -> str:
    ...
    def generate_wfs_url(self, layer_name: str, workspace: str = None,
    ...
    def sync_layer_with_database(self, layer_name: str, table_name: str,
