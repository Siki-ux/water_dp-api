...
from app.schemas.time_series import (
    TimeSeriesQuery, TimeSeriesAggregation, InterpolationRequest,
    AggregatedDataPoint, TimeSeriesStatistics
)
...
class TimeSeriesService:
    ...
    def get_time_series_data(self, query: TimeSeriesQuery) -> List[TimeSeriesData]:
        ...
        try:
            ...
            if query.quality_filter:
                db_query = db_query.filter(TimeSeriesData.quality_flag == query.quality_filter)
            if not query.include_interpolated:
                db_query = db_query.filter(TimeSeriesData.is_interpolated == False)
            if not query.include_aggregated:
                db_query = db_query.filter(TimeSeriesData.is_aggregated == False)
    ...
    def aggregate_time_series(self, aggregation: TimeSeriesAggregation) -> List[AggregatedDataPoint]:
        ...
        try:
            ...
            if aggregation.time_zone != 'UTC':
                df['timestamp'] = df['timestamp'].dt.tz_localize('UTC').dt.tz_convert(aggregation.time_zone)
            ...
            if aggregation.aggregation_method == 'mean':
                ...
                raise ValueError(f"Unsupported aggregation method: {aggregation.aggregation_method}")
    ...
    def interpolate_time_series(self, request: InterpolationRequest) -> List[TimeSeriesData]:
        ...
        try:
            ...
            if request.method == 'linear':
                ...
                interpolated_values = df_reindexed['value'].interpolate(method='spline', order=3)
            elif request.method == 'polynomial':
                interpolated_values = df_reindexed['value'].interpolate(method='polynomial', order=2)
            else:
                interpolated_values = df_reindexed['value'].interpolate(method='linear')
            ...
            for timestamp, value in interpolated_values.items():
                if pd.notna(value):
                    ...
                    interpolated_points.append(TimeSeriesData(
                        series_id=request.series_id,
                        timestamp=timestamp,
                        value=float(value),
    ...
    def detect_anomalies(self, series_id: str, start_time: datetime, 
                        end_time: datetime, method: str = 'statistical',
    ...
    def calculate_statistics(self, series_id: str, start_time: datetime,
                           end_time: datetime) -> TimeSeriesStatistics:
        ...
        try:
            ...
            return TimeSeriesStatistics(
                ...
                metadata={
                    ...
                    'data_quality_score': self._calculate_quality_score(quality_summary)
                }
            )
    ...
    def export_time_series(self, series_id: str, start_time: datetime,
